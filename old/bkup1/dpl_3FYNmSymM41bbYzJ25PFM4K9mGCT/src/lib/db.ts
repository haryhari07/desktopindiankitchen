...
function getOptimizedImageUrl(url: string): string {
  ...
  const webpPath = path.join(process.cwd(), 'public', webpUrl);
...
function mapGeneratedRecipe(row: GeneratedRecipeRow): GeneratedRecipe {
  return {
    id: row.id,
    userId: row.user_id,
    title: row.title,
...
export const db = {
  ...
  createGeneratedRecipe: async (
    userId: string,
    data: Omit<GeneratedRecipe, 'id' | 'userId' | 'createdAt'>
  ) => {
    await ensureTables();
    const id = crypto.randomUUID();
    const createdAt = new Date().toISOString();
  ...
  getUserGeneratedRecipes: async (userId: string) => {
    await ensureTables();
    const result = await sql<GeneratedRecipeRow>`
  ...
  getGeneratedRecipeCountForUser: async (userId: string) => {
    await ensureTables();
  ...
  deleteGeneratedRecipeForUser: async (userId: string, id: string) => {
    await ensureTables();
    const result = await sql<GeneratedRecipeRow>`
  ...
  deleteOldestGeneratedRecipeForUser: async (userId: string) => {
    await ensureTables();
